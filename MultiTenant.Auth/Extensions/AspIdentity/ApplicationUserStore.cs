...
namespace MultiTenantAuth.Extensions.AspIdentity
{
    public class ApplicationUserStore<TUser, TTenant> : UserStore<
        ApplicationUser, ApplicationRole, ApplicationIdentityDbContext, string, IdentityUserClaim<string>, ApplicationUserRole,
        ...
        where TTenant : Tenant
    {
            ...
            CancellationToken cancellationToken = default)
        {
            var query = from userRole in Context.Set<ApplicationUserRole>()
                        join role in Context.Set<ApplicationRole>() on userRole.RoleId equals role.Id
                        where userRole.UserId.Equals(user.Id) && userRole.TenantId.Equals(tenant.Id) && userRole.State == States.Active
                        select role.Name;
